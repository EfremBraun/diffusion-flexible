...
new_data_directory='adsorption_isotherms'

function display_help() {
    echo "___ finddata ___"
    echo 
    echo " Will consolidate datafile from RASPA-simulations"
    echo  
    echo " This can be used to extract nice data files with data"
    echo
    echo " Usage $0 options"
    echo 
    echo "OPTIONS:"
    echo " -h       show this message"
    echo " -c       Name of component"
    echo 
}
...
function getdata() {
    ...
    do
        ...
        pp=$(grep "Component [0-9] \[$comp\] (Adsorbate molecule)" -A40 $1 |\
             grep "Partial pressure" | awk '{print $3}')

        fc=$(grep "Component [0-9] \[$comp\] (Adsorbate molecule)" -A40 $1 |\
             grep "Fugacity coefficient" | awk '{print $3}')

        pf=$(grep "Component [0-9] \[$comp\] (Adsorbate molecule)" -A40 $1 |\
...
error_messages=0
for file in $(find . -name "output_*.data")
do
    getdata $file
done
