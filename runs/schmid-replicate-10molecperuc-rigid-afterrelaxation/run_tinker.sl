...
#SBATCH --qos premium
#SBATCH -n 1
#SBATCH --job-name schmid-replicate-10molecperuc-rigid-afterrelaxation
...
# freeze MOF atoms, then equilibrate some more
echo inactive -$(expr $benzene_molecules \* 12 + 1) $(expr $benzene_molecules \* 12 + $mof_unitcells \* 424) >> $output_key_file # freeze MOF atoms
$bin/dynamic MOFwithBenzene 500000 1 0.1 2 300.0 | tee MOFwithBenzene-3.5.log
#head -n -1 $output_key_file > temp_key_file; mv temp_key_file $output_key_file # unfreeze MOF atoms
mv MOFwithBenzene.arc MOFwithBenzene-3.5.arc
cp MOFwithBenzene.dyn MOFwithBenzene-3.5.dyn
sed -i '/25.945700   25.945700  129.728500   90.000000   90.000000   90.000000/d' MOFwithBenzene-3.5.arc # clean up lines that VMD can't read
